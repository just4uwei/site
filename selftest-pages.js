...
function render(url) {
  const dom = execFileSync(BROWSER, [
    ...
    // 必须有超时：execFileSync 默认无限等，浏览器一挂住整个自检就没了下文
    timeout: 30000,
  });
...
function check(name, url, want) {
  let text;
  try {
    text = render(BASE + url);
  } catch (e) {
    failed++;
...
(async () => {
  ...
  check('首页', '/', ['游戏作品', 'works冒烟条目', 'news冒烟条目', '闽ICP备']);
  check('作品列表', '/works.html', ['游戏作品', 'works冒烟条目']);
  check('App 列表', '/apps.html', ['apps冒烟条目']);
  check('资讯列表', '/news.html', ['news冒烟条目']);
  check('关于', '/about.html', ['关于与联系', '不开放评论与留言']);
  check('作品详情', `/work.html?id=${ids.works}`, ['works冒烟条目', '第二段正文']);
  check('App 详情', `/app.html?id=${ids.apps}`, ['apps冒烟条目', '第二段正文']);
  check('资讯详情', `/post.html?id=${ids.news}`, ['news冒烟条目', '第二段正文']);
  check('管理台登录', '/admin.html', ['管理目标', '管理后台']);
  // 详情页拿到不存在的 id 时，要好好说一句人话，而不是抛异常白屏
  check('详情页 id 不存在', '/work.html?id=999999', ['内容不存在或未发布']);
  // 干净地址（/works 无扩展名）与 404 页
  check('干净地址', '/works', ['works冒烟条目']);
  check('404 页', '/nope-nope', ['这个地址没有内容']);
