...
- **巨字字号按字数算**：`--n` 由 `site.js` 按标题长度注入，CSS 里
  ...
  站名从三个字改成四个字就直接溢出屏幕。用默认月相图时收到 `66vw`
  （`.hero-stage.has-sky`）给图让位——满幅的巨字会把图整张盖死。
- **首屏默认主视觉是手写的月相 SVG**（`site.js` 的 `heroSky()`）：一轮月从
  新月长到满月排在一条轨道上，七个相位＝七个晚上，呼应「一两个晚上玩完」。
  终止线按真几何算（椭圆 x 半轴 `r·|1-2f|`），路径由脚本算好写死，运行时不算三角。
  颜色一律取 `site.css` 的 token，没有写死色值。后台传了 `hero_image_id`
  就改用那张照片。两者叠法相反：**照片压在巨字上，月相图垫在巨字下**——
  月相图是暗底上的几个亮点，压上去只会把字糊掉。
- **首屏名片可以没有**：`author_note` 留空时整张名片不出现。
  设置项默认就是空的，别假设它们有值。
